...
#include <iostream>
#include <cassert>
#include "slr-ring-routing-agent.h"
#include "scheduler.h"
#include "utils.h"
#include "node.h"
#include "world.h"
...
using SLRInitialisationGenerationEvent = CallMethodEvent<SLRRingRoutingAgent,
  &SLRRingRoutingAgent::processPacketGenerationEvent,
...
//==============================================================================
//
//          SLRRingRoutingAgent  (class)
//
//==============================================================================

ofstream SLRRingRoutingAgent::SLRPositionsFile;
mt19937_64 *SLRRingRoutingAgent::forwardingRNG =  new mt19937_64(1);
int SLRRingRoutingAgent::currentAnchorID = 0;
int SLRRingRoutingAgent::forwardedSLRBeacons = 0;


SLRRingRoutingAgent::SLRRingRoutingAgent(Node *_hostNode) : RoutingAgent(_hostNode) {
  type = RoutingAgentType::SLR_RING;
...
SLRRingRoutingAgent::~SLRRingRoutingAgent() {
  ...
  if ( aliveAgents == 0 ) {
    ...
    cout << " SLR Routing with ring:: forwarded packets: " << forwardedDataPackets << endl;
    cout << " SLR Routing with ring:: forwarded beacons: " <<  forwardedSLRBeacons << endl;
  }
  //    if ( SLRCoordX == -1 || SLRCoordY == -1  || SLRCoordZ == -1 ){
  //// if ( hostNode->getId() < 30  ){
  //        cout << "SLRRingRoutingAgent destructor on Node " << hostNode->getId() ;
  //        cout << " SLR coord " << SLRCoordX << " " << SLRCoordY << " " << SLRCoordZ ;
  //        cout << endl;
  //    }
}

void SLRRingRoutingAgent::initializeAgent() {
  string filename;
...
void SLRRingRoutingAgent::receivePacketFromNetwork(PacketPtr _packet) {
  ...
  //          cout << Scheduler::getScheduler().now() << " SLRRingRoutingAgent::receivePacket on Node " << hostNode->getId();
 ...
 if (_packet->type == PacketType::CONTROL_1 || _packet->type == PacketType::CONTROL_2) {
  ...
  if ( forward ) { 
	...
	if (nodeIt == ring.end()) {
		...
		ring.insert(pair<int,Contro12_t>(_packet->transmitterId, control));
...
bool SLRRingRoutingAgent::isCloser(PacketPtr _packet){
  bool res = false;
...
bool SLRRingRoutingAgent::SLRForward(PacketPtr _packet, int m )
{
...
bool SLRRingRoutingAgent::SLRForward(PacketPtr _packet)
{
...
bool SLRRingRoutingAgent::SLRDestinationReach(PacketPtr _packet)
{
...
void SLRRingRoutingAgent::receivePacketFromApplication(PacketPtr _packet) {
  ...
  if (_packet->type == PacketType::DATA && _packet->dstId != -1 ){
    ...
    SLRRingRoutingAgent* dstAgent = dynamic_cast<SLRRingRoutingAgent*> (dst->getRoutingAgent());
...
bool SLRRingRoutingAgent::SLRUpdate(PacketPtr _packet)
{
  int anchorId = _packet->anchorID;
  int anchorDist =  _packet->anchorDist;
  bool update=false;

  SLRRingRoutingAgent* routing = static_cast<SLRRingRoutingAgent*> (hostNode->getRoutingAgent());
...
void SLRRingRoutingAgent::processPacketGenerationEvent() {
