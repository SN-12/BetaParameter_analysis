...
#include <iostream>
#include "scheduler.h"
#include "proba-flooding-ring-routing-agent.h"
...
mt19937_64 *ProbaFloodingRingRoutingAgent::forwardingRNG =  new mt19937_64(2);
set<int> ProbaFloodingRingRoutingAgent::reachability = set<int>();

ProbaFloodingRingRoutingAgent::ProbaFloodingRingRoutingAgent(Node *_hostNode) : RoutingAgent(_hostNode) {
  type = RoutingAgentType::PROBA_FLOODING_RING;
...
ProbaFloodingRingRoutingAgent::~ProbaFloodingRingRoutingAgent() {
  aliveAgents--;
  //   cout << " aliveAgents " << aliveAgents << endl;

  if ( aliveAgents == 0 ){
    //           TODO : add global information
    cout << " Proba flooding with ring:: number of nodes reached : " << reachability.size() << endl;
    cout << " Proba flooding with ring:: forwarded packets       : " << forwardedDataPackets << endl;
  }
}


void ProbaFloodingRingRoutingAgent::receivePacketFromApplication(PacketPtr _packet) { //on source
...
void ProbaFloodingRingRoutingAgent::receivePacketFromNetwork(PacketPtr _packet) {
...
void ProbaFloodingRingRoutingAgent::forwardPacketIfOnRing( PacketPtr _packet ) {

	auto nodeIt = ring.find( _packet->transmitterId );
	if (nodeIt == ring.end()) {
		// first time we see a packet coming from this node	
		Controlll12_t control;
		...
		ring.insert(pair<int,Controlll12_t>(_packet->transmitterId, control));
